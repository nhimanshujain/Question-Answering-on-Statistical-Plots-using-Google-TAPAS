...
def run(png_dir, detections_dir, csv_dir, MIN_CLASS_CONFIDENCE=0.8):
    ...
    for _ in tqdm(range(len(image_names))):
            ...
            if class_name in textual_elements:
                ...
                if emptyBar:
                    ...
                    if negativeBar:
                        ...
                        c = get_color(image.crop(bb), plot_type)
                        image_data.append({
                            "bbox" : bb,
                            "pred_class" : class_name,
                            "confidence" : score,
        ...
        xlabel = list(set([dd["ocr_text"] for dd in image_data if dd["pred_class"]=="xlabel"]))[0]
        ylabel = list(set([dd["ocr_text"] for dd in image_data if dd["pred_class"]=="ylabel"]))[0]
        if isHbar:
        ...
        if isSinglePlot:
            if isHbar:
                visual_data = [(dd['x_value'], dd['y_value'], xlabel) for dd in image_data if dd["pred_class"] in ['dot_line', 'bar', 'line']]
            else:
                visual_data = [(dd['x_value'], dd['y_value'], ylabel) for dd in image_data if dd["pred_class"] in ['dot_line', 'bar', 'line']]
        else:
            visual_data = [(dd['x_value'], dd['y_value'], dd['associated_label']) for dd in image_data if dd["pred_class"] in ['dot_line', 'bar', 'line']]
        ...
        # try:
        #     ylabel_str = ylabel.decode('utf-8')
        #     df["ylabel"] = [ylabel_str]*len(df)
        # except:
        #     df["ylabel"] = [ylabel]*len(df)
    
        # try:
        #     xlabel_str = xlabel.decode('utf-8')
        #     df["xlabel"] = [xlabel_str]*len(df)
        # except:
        #     df["xlabel"] = [xlabel]*len(df)

        # try:
        #     title_str = title.decode('utf-8')
        #     df["title"] = [title_str]*len(df)
        # except:
        ...
        #   try:
        #       legend_orientation_str = legend_orientation.decode('utf-8')
        #       df["legend orientation"] = [legend_orientation_str]*len(df)
        #   except:
