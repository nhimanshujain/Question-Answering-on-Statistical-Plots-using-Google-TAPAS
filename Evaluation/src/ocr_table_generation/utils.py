...
def get_color(img, plot_type, color_range=512):
    """
    ...
    try:
        if plot_type == "dot_line" or plot_type == "line":
            for c in colors:
            ...
            for c in colors:
                if c[1] not in [(255,255,255), (0,0,0), 0, (0)] and c[1]!=0 and colorDistance(c[1], [255,255,255],method="euclidian") > 50:
                    if c[0] > max_occurence:
                        (max_occurence, most_present) = c
                    elif abs(max_occurence - c[0]) <= 10 :
                        new_c1 = c[1]
                        most_present = (round((new_c1[0] +  most_present[0])/2), round((new_c1[1] +  most_present[1])/2), round((new_c1[2] +  most_present[2])/2) )
        ...
        if color_range < 10000:
            return get_color(img, plot_type, color_range)
...
def match_colors(group, _mapping):
    ...
    for dd in group:
        ...
        # print([(lbl, colorDistance(c, visual_color, method="euclidian") )for lbl,c in mapping.items()])
...
def associate_bar_legend(image_data, isHbar):
    ...
    # print("VISUAL DATA:", visual_groups)
